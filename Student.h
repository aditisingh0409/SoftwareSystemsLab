...
void enroll(int clientSocket,char* auth) {
    ...
    send(clientSocket, "Enter the Course ID\n", strlen("Enter the Course ID\n"), 0);
        ...
        int fd2 = open("Courses.txt", O_RDWR);
        if (fd2 == -1) 
        {
            perror("Error opening the file");
            return;
        ...
        if(cfound)
        {
            ...
            if (avail_seats > 0) 
            {           
            	// Open the Enrollments.txt to check if student already enrolled for any course or not
    		int fd1 = open("Enrollments.txt", O_RDWR | O_CREAT, 0666); // Open the file in append mode
    		if (fd1 == -1) 
    		{
       			perror("Error opening the file");
       			return ;
                ...
                {   
                    ...
                    if (numEnrollments < 2)
                    {
                        ...
                        if (write(fd2, &course_record, sizeof(struct course)) == -1) 
                        {
                            ...
                            send(clientSocket, "Error Updating Courses.txt\n", strlen("Error Updating Courses.txt\n"), 0);
                            return;
                        } 
                        else 
                        {                
                            ...
                            {
                                perror("Error writing to the file");
                                send(clientSocket, "Error Updating Enrollments.txt\n", strlen("Error Updating Enrollments.txt\n"), 0);
                                return;
                            }
...
void drop(int clientSocket, char* auth) {
    ...
    int fd1 = open("Enrollments.txt", O_RDWR); // Open the file for read and write
    int fd2;
    ...
    if (sfound) 
    {
        fd2 = open("Courses.txt", O_RDWR); // Open the course file for read and write
        if (fd2 == -1) 
        {
...
int updatePassword(int clientSocket, char* auth) {
    char buff[1024];
    int fd = open("Students.txt", O_RDWR); // Open the file for both reading and writing
    if (fd == -1) 
    {
        perror("Error opening the file");
        return false;
